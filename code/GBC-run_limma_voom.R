...
for(base_name in base_names){
  ...
  count.matrix<-t(counts)
  nf <- edgeR::calcNormFactors(count.matrix, method = 'TMM')
  
  mod <- model.matrix(~Group+Batch, data=cellinfo)
  
  voom.data <- limma::voom(count.matrix, design = mod, lib.size = colSums(count.matrix) * nf)
  ...
  rownames(result.table) <- rownames(count.matrix)
  #can check up/down genes with logFC ex) upDE=which(result.table$adjpvalue<0.05&result.table$logFC>1)
  #downDE=which(result.table$adjpvalue<0.05&result.table$logFC<1)
  voomDE=rownames(result.table)[which(result.table$adjpvalue<0.05)]
  voomDown=rownames(result.table)[which(result.table$adjpvalue<0.05&result.table$logFC>0)]
  voomUp=rownames(result.table)[which(result.table$adjpvalue<0.05&result.table$logFC<0)]
